...
public class AnalysisService {
    ...
    public JsonNode getAnalysis(AnalysisRequestDto analysisRequestDto) {
         ...
         * "feedback":
         * "하루 종일 많이 힘드셨을 것 같아요. 자신을 탓하지 마시고 오늘은 충분히 쉬어 주세요. 내일은 조금 더 나은 날이 될 거예요.",
         * "recommendations": [
         * {
         * "type": "music",
         * "title": "이수 - 사랑이라는 이유로",
        ...
        String prompt = String.format(
                """
                        ...
                        \"""
                        """,
                contentRepository.findNameByContentId(analysisRequestDto.getContent()), analysisRequestDto.getMemo());
        ...
        try {
            OpenAiResponse response = restTemplate.postForObject(API_URL, entity, OpenAiResponse.class);
            String content = Objects.requireNonNull(response).getChoices().get(0).getMessage().getContent()
                    .replace("```json", "").replace("```", "").trim();

            log.info("OpenAI raw response: {}", content);

            // OpenAI가 거부했는지 확인
            if (content.startsWith("I'm sorry") || content.startsWith("I can't") || content.startsWith("I cannot")) {
                // 더미 응답 반환
                String fallbackResponse = "{\n" +
                        "\"emotions\": [\n" +
                        "  {\"code\": 1, \"name\": \"기쁨\", \"score\": 5},\n" +
                        "  {\"code\": 2, \"name\": \"슬픔\", \"score\": 3}\n" +
                        "],\n" +
                        "\"analysis\": \"죄송합니다. 현재 AI 분석 서비스에 일시적인 문제가 있습니다. 나중에 다시 시도해주세요.\"\n" +
                        "}";

                ObjectMapper objectMapper = new ObjectMapper();
                return objectMapper.readTree(fallbackResponse);
            }
            ...
            log.error("OpenAI 분석 실패: {}", e.getMessage());
            ...
            String fallbackResponse = "{\n" +
                    ...
                    "  {\"code\": 1, \"name\": \"기쁨\", \"score\": 5}\n" +
                    "],\n" +
                    "\"analysis\": \"AI 분석 중 오류가 발생했습니다. 메모는 정상적으로 저장되었습니다.\"\n" +
                    "}";
            ...
            try {
                ...
                throw new RuntimeException("분석 서비스 오류", ex);
            }
        }
    }
    ...
    public void save(AnalysisSaveDto analysisSaveDto) {
        ...
        List<AnalysisTag> analysisTags = analysisSaveDto.getEmotions().stream().map(emotion -> AnalysisTag.builder()
